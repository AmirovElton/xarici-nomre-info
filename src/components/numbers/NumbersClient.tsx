...
import { Search, Send, Globe, RefreshCw, ArrowLeft } from 'lucide-react'
...
export default function NumbersClient() {
          ...
          <div className="grid grid-cols-2 gap-4 max-w-md mx-auto mb-4">
            {platforms.slice(0, 2).map((platform) => (
              <button
                ...
                className="glass-card p-6 flex flex-col items-center gap-3 hover:border-indigo-500/40 hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
              >
                <span className="text-4xl">{platform.icon}</span>
                <span className="font-semibold text-gray-100">{platform.name}</span>
                <span className="text-xs text-gray-500">{platform.desc}</span>
              </button>
            ))}
          </div>
          <div className="grid grid-cols-1 gap-4 max-w-[calc(50%-0.5rem)] mx-auto">
  ...
  // Country list after platform selection
  return (
    <div className="px-4 py-6 animate-fade-in">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-6">
          <button
            ...
            className="inline-flex items-center gap-2 text-sm text-indigo-400 hover:text-indigo-300 mb-3 transition-colors"
          >
            <ArrowLeft size={16} /> Platformalar
          </button>
          <h1 className="section-title">
            {platforms.find(p => p.id === selectedPlatform)?.name} Nömrələri
          </h1>
        ...
        <div className="relative max-w-lg mx-auto mb-5">
          ...
          <input
            ...
            placeholder="Ölkə adı və ya ölkə kodu axtar..."
            className="w-full pl-11 pr-4 py-3.5 rounded-2xl bg-zinc-900/80 border border-zinc-700/50 text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500/40 transition-all"
          />
        </div>
            ...
            <button
              ...
              className={cn(
                ...
                stockFilter === filter.id
                  ...
                  : 'bg-zinc-900/50 text-gray-400 border-zinc-700/50 hover:bg-zinc-800/50'
              )}
            >
              {filter.label}
            </button>
